...
if (-not $webView) { throw "WebView2 offline installer not found" }
...
& $compiler (Join-Path $repo "installer\SilenceCutter-Lightweight.iss")
if ($LASTEXITCODE -ne 0) { throw "Lightweight installer build failed with exit code $LASTEXITCODE" }
Get-FileHash (Join-Path $repo "release\SilenceCutter-Lightweight-Setup.exe") -Algorithm SHA256
