...
class FormatterRendererTests(unittest.TestCase):
    ...
    def test_filter_uses_planned_crop_without_stretch_and_expected_geometry(self):
        ...
        self.assertIn("asetrate=48000*1.100000000", graph)
        self.assertIn("atempo=0.909090909", graph)
        self.assertIn("highpass=f=90", graph)
        self.assertIn("afftdn=nr=18", graph)
        self.assertIn("lowpass=f=14000", graph)
        self.assertIn("acompressor=threshold=0.12", graph)
    ...
    def test_ffmpeg_render_outputs_dimensions_codecs_and_duration(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result["pitch_ratio"], 1.10)
            self.assertTrue(result["voice_enhance_enabled"])
            self.assertEqual(result["audio_filter_profile"]["highpass_hz"], 90)
            self.assertEqual(result["audio_filter_profile"]["noise_reduction"], 18)
